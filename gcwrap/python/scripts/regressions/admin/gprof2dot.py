    ...
    def _integrate_cycle(self, cycle, outevent, inevent):
        if outevent not in cycle:
            ...
            for member in cycle.functions:
                ...
                for call in list(member.calls.values()):
                    callee = self.functions[call.callee_id]
            ...
            for callee, call_ratio in list(callees.items()):
                ranks = {}
                call_ratios = {}
                partials = {}
...
class GprofParser(Parser):
    ...
    def translate(self, mo):
        ...
        for name, value in list(groupdict.items()):
            if value is None:
                value = None
    ...
    def parse(self):
        ...
        cycles = {}
        for index in list(self.cycles.keys()):
            cycles[index] = Cycle()

        for entry in list(self.functions.values()):
...
class OprofileParser(LineParser):
    ...
    def update_subentries_dict(self, totals, partials):
        for partial in list(partials.values()):
            try:
    ...
    def parse(self):
        ...
        profile[SAMPLES] = 0
        for _callers, _function, _callees in list(self.entries.values()):
            ...
            for _callee in list(_callees.values()):
...
class SharkParser(LineParser):
    ...
    def parse(self):
        ...
        profile = Profile()
        profile[SAMPLES] = 0
        for _function, _callees in list(self.entries.values()):
            function = Function(_function.id, _function.name)
            function[SAMPLES] = _function.samples
            profile.add_function(function)
            profile[SAMPLES] += _function.samples
...
class PstatsParser:
    ...
    def parse(self):
        ...
        for fn, (cc, nc, tt, ct, callers) in list(self.stats.stats.items()):
            ...
            for fn, value in list(callers.items()):
