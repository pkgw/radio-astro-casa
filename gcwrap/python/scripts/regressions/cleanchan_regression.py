...
for i in range(len(lname)):
    if pyonly:
        ...
        print(fmt % lname[i], "%4i %20s %20s" % stats[i], file=logfile)
    else:
        print(fmt % lname[i], "%15.7f %11.7f %10.7f %10.7f %4i %16.7f %16.7f" % stats[i], file=logfile)
...
if pyonly:
    ...
    for i in range(1,len(lname)):
        ...
        if stats[i][4]>0:
            ...
            print(fmt % lname[i], "%15.7f %10.7f %10.7f %10.7f %4i %16.7f %16.7f" % (foo[0],foo[1],foo[2],foo[3],foo[4],foo[5],foo[6]), file=logfile)
        else:
            print(fmt % lname[i], " FAIL")
            print(fmt % lname[i], " FAIL", file=logfile)
        ...
        for te in range(len(tests)):
            ...
            if tests[te]=='width' or tests[te]=='ch0-ch1' or tests[te]=='chn-ch(n-1)':
                if sname[run]=='nvel' or sname[run]=='frm':
                    adiff=abs(-stats[run][te]-stats[0][te])/stats[0][te]
            if adiff < tol:
                print("* Passed %10s %-10s test, got %-11.5g expected %-11.5g" % (sname[run],tests[te],stats[run][te],stats[0][te]), file=logfile)
            else:
                print("* FAILED %10s %-10s test, got %-11.5g expected %-11.5g " % (sname[run],tests[te],stats[run][te],stats[0][te]), file=logfile)
                regstate = False
